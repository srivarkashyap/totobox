...
class TotoBoxExtension {
    constructor(context) {
        ...
        this.chartTimeWindow = '7d';
        this.totalApiCalls = 0;
        this.hasRealKeys = false;
        this.cryptoUtils = crypto_utils_1.CryptoUtils.getInstance();
        this.configManager = config_manager_1.ConfigManager.getInstance(context);
    ...
    async fetchAnalytics() {
        try {
            ...
            if (res?.success && apiResponse?.success && apiResponse?.data?.today) {
                ...
                if (Date.now() - this.lastSuccessfulUpdate > 10 * 60000) {
    ...
    determineOptimalTimeWindow(callCount) {
        if (callCount === 0) {
        ...
        const chartDataPromises = keys.map(async (key, index) => {
            try {
                ...
                const [tokenResponse, costResponse, callsResponse] = await Promise.all([
                    http_client_1.httpClient.get(`${backendUrl}/api/charts?user_id=${encodeURIComponent(key.id)}&chart_type=token_usage&days=${daysParam}`, {
                        timeout: this.configManager.get('timeout'),
                        cache: true,
                        cacheTTL: 300000
                    }),
                    http_client_1.httpClient.get(`${backendUrl}/api/charts?user_id=${encodeURIComponent(key.id)}&chart_type=cost_trend&days=${daysParam}`, {
                        timeout: this.configManager.get('timeout'),
                        cache: true,
                        cacheTTL: 300000
                    }),
                    http_client_1.httpClient.get(`${backendUrl}/api/charts?user_id=${encodeURIComponent(key.id)}&chart_type=hourly_pattern&days=${daysParam}`, {
                        timeout: this.configManager.get('timeout'),
                        cache: true,
                        cacheTTL: 300000
                    })
    ...
    generateChartLabels(timeWindow) {
        if (timeWindow === '1h') {
    ...
    extractDailyTokens(chartData, labels) {
        const arrayLength = labels.length;
    ...
    extractDailyCosts(chartData, labels) {
        const arrayLength = labels.length;
    ...
    extractDailyCalls(chartData, labels) {
        const arrayLength = labels.length;
    ...
    registerCommands() {
        const showDashboardCmd = vscode.commands.registerCommand('totobox.showDashboard', () => {
            console.log('Show Dashboard command triggered');
            return this.showDashboard();
        });
        const refreshDataCmd = vscode.commands.registerCommand('totobox.refreshData', () => {
    ...
    async showDashboard() {
        ...
        this.webviewPanel = vscode.window.createWebviewPanel('totoboxDashboard', 'totoboX Dashboard', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
        this.webviewPanel.onDidDispose(() => {
            this.webviewPanel = null;
        });
        this.webviewPanel.webview.onDidReceiveMessage(async (msg) => {
            try {
                switch (msg?.command) {
                    case 'switchTab':
    ...
    async handleToggleKeySelection(keyId, chartType) {
    ...
    async handleChangeTimeWindow(timeWindow) {
    ...
    async buildHTMLContent(nonce, keys, activeKey, chartDataSets) {
        return `
            <!DOCTYPE html>
            <html lang="en">
            <head>
    ...
    async getTabContent(keys, activeKey, chartDataSets) {
    ...
    getHomeContent(keys, activeKey, chartDataSets) {
        return `
                        ...
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <select id="provider" onchange="updateVersions()">
                                    ${Object.entries(PROVIDERS).map(([key, val]) => `<option value="${key}">${val.label}</option>`).join('')}
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select id="providerVersion">
                                    ${PROVIDERS.openai.versions.map(v => `<option value="${v}">${v}</option>`).join('')}
                                </select>
                            </div>
                        </div>
    ...
    getJavaScript(nonce, chartDataSets) {
    ...
    async handleGenerateProxy(data) {
        ...
        try {
            ...
            const generateCleanId = (length = 7) => {
            ...
            const response = await http_client_1.httpClient.postJson(`${backendUrl}/api/register`, {
                userId: userId,
                provider: provider,
                apiKey: apiKey
            });
            ...
            const selection = await vscode.window.showInformationMessage('✅ Proxy key generated successfully!', 'Copy Proxy Key', 'Copy Base URL', 'View Instructions');
    ...
    showProxyInstructions(proxyKey, proxyUrl) {
        const panel = vscode.window.createWebviewPanel('proxyInstructions', 'TotoboX Proxy Setup', vscode.ViewColumn.One, {});
        panel.webview.html = `
            <!DOCTYPE html>
            <html lang="en">
            <head>
    ...
    async handleSwitchKey(keyId) {
        const keys = await this.getAPIKeys();
        keys.forEach(k => k.isActive = k.id === keyId);
        await this.setAPIKeys(keys);
        await this.refreshWebview();
    }
    async handleRemoveKey(keyId) {
        const keys = await this.getAPIKeys();
    ...
    async handleRenameKey(keyId, newName) {
        const keys = await this.getAPIKeys();
    ...
    async handleCopyProxy(proxyKey) {
        await vscode.env.clipboard.writeText(proxyKey);
        if (this.configManager.get('showNotifications')) {
            vscode.window.showInformationMessage('Proxy key copied to clipboard');
        }
    ...
    async handleExportCSV() {
        try {
            ...
            if (saveUri) {
                ...
                vscode.window.showInformationMessage(`✅ Exported ${logs.length} logs to ${filename}`, 'Open File').then(selection => {
